...
class StartingNewFlowState(State):
  ...
  def __init__(self, migration, controller, migration_config, source_nodes, sink_nodes, insertion_nodes):
    ...
    self._node_id_to_new_flow_state = {}

  def initialize(self):
    for source_id in self._source_nodes.keys():
      self._node_id_to_new_flow_state[source_id] = 'pending'

    for sink_id, sink in self._sink_nodes.items():
      self._migration.send(sink, messages.migration.start_flow(self._migration.id))
      self._node_id_to_new_flow_state[sink_id] = 'pending'
  ...
  def _maybe_finish(self):
    if all(state == 'completed_flow' for state in self._node_id_to_new_flow_state.values()):
      self._migration.finish_state_starting_new_flow()

  def receive(self, message, sender_id):
    if message['type'] == 'started_flow':
      self._node_id_to_new_flow_state[sender_id] = 'completed_flow'
      self._maybe_finish()
    else:
