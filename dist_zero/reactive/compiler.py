...
class ReactiveCompiler(object):
  ...
  def compile(self, output_key_to_norm_expr):
    ...
    The input program is provided as a dictionary that maps output keys to `dist_zero.reactive.expression.Expression` instances.
    ...
    Passing in and receiving states:
      ...
      `dist_zero.reactive.expression.Expression`.
    ...
    Passing in and receiving transitions:
      ...
      expression.  The calculated transitions will be exactly those determined by the associated `dist_zero.reactive.expression.Expression`
    ...
    :type output_key_to_norm_expr: dict[str, dist_zero.reactive.expression.Expression]
  ...
  def capnp_state_builder(self, expr):
    ...
    :type expr: `dist_zero.reactive.expression.Expression`
...
class _Topsorter(object):
  '''
  Helper class to populate ReactiveCompiler._top_exprs via topological traversal of `dist_zero.reactive.expression.Expression`
  instances referenced by a list of root expressions.
  '''

  def __init__(self, root_exprs):
    '''
    :param list root_exprs: A list of `dist_zero.reactive.expression.Expression` instances in any order.
    '''
