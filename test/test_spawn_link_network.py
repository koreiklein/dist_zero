...
class TestSpawnSumLinkNetwork(Utils):
  async def _connect_and_test_io_trees(self, n_input_leaves, n_output_leaves):
    root_input = await self.root_io_tree(machine=self.machine_ids[0], leaf_config=messages.io.sum_leaf_config(0))
    self.root_input = root_input
    root_output = await self.root_io_tree(machine=self.machine_ids[0], leaf_config=messages.io.sum_leaf_config(0))
