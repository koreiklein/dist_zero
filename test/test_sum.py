...
def test_send_no_transport(simulated_system):
  ...
  node_a = machine.start_node(messages.io.internal_node_config(dist_zero.ids.new_id(), variant='input'))
  node_b = machine.start_node(
      messages.io.internal_node_config(dist_zero.ids.new_id(), variant='output', initial_state=0))
...
def test_sum_two_nodes_on_three_machines(demo):
  ...
  demo.system.send_to_node(sum_node_handle,
                           messages.migration.set_output(root_output_node_handle,
                                                         demo.system.create_transport_for(
                                                             sum_node_handle, root_output_node_handle)))
